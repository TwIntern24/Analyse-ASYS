...
#include "DataMatrix_ZXing/DMBitLayout.h"

#include "DataMatrix_ZXing/BitArray.h"
#include "DataMatrix_ZXing/BitMatrix.h"
#include "DataMatrix_ZXing/ByteArray.h"
#include "DataMatrix_ZXing/DMVersion.h"
...
template <typename VisitFunc>   //← VisitMatrix gets a functor/callback that is responsible for processing/visiting the bits in the matrix.
BitMatrix VisitMatrix(int numRows, int numCols, VisitFunc visit)    //The return value contains a BitMatrix where each bit that has been visited is set.
{
...
BitMatrix BitMatrixFromCodewords(const ByteArray& codewords, int width, int height) //← Symbol Character Placement Program. Adapted from Annex M.1 in ISO/IEC 16022:2000(E).
{
...
static BitMatrix ExtractDataBits(const Version& version, const BitMatrix& bits) //← Extracts the data bits from a BitMatrix that contains alignment patterns.
{
...
//↓ Reads the bits in the {@link BitMatrix} representing the mapping matrix (No alignment patterns) in the correct order
//in order to reconstitute the codewords bytes contained within the Data Matrix Code; return bytes encoded within the Data Matrix Code
