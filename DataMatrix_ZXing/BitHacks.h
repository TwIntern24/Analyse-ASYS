...
#if defined(ZX_HAS_GCC_BUILTINS)
    return __builtin_clz(x); // Use GCC built-in function for counting leading zeros
#elif defined(ZX_HAS_MSC_BUILTINS)
    return __lzcnt(x); // Use MSVC built-in function for counting leading zeros
#else
    // Fallback implementation for platforms without built-in functions
    int n = 0;
    if (sizeof(x) <= 4) {
        if (x == 0)
            return 32;
...
inline int NumberOfTrailingZeros(T v) {
    assert(v != 0);
#if defined(ZX_HAS_GCC_BUILTINS)
    return __builtin_ctz(v); // Use GCC built-in function for counting trailing zeros
#elif defined(ZX_HAS_MSC_BUILTINS)
    unsigned long where;
#if defined(_WIN64)
