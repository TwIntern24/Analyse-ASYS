...
#include "DataMatrix_ZXing/Pattern.h"
...
template<typename T>
bool isSet(T v) { return bool(v); }

bool BitMatrix::getTopLeftOnBit(int& left, int& top) const
{
    int bitsOffset = (int)std::distance(_bits.begin(), std::find_if(_bits.begin(), _bits.end(), isSet<unsigned char>));
...
{
    int bitsOffset = Size(_bits) - 1 - (int)std::distance(_bits.rbegin(), std::find_if(_bits.rbegin(), _bits.rend(), isSet<unsigned char>));
	if (bitsOffset < 0) {
		return false;
	}
