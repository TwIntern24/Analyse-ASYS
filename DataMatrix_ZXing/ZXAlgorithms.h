...
//↓ Forward declaration of the template class 'C'
template <typename... Ts>
class C;

//↓ Define a helper function to get the value type of the container
template <typename Container>
typename Container::value_type GetDefaultValue() {
    return typename Container::value_type();
}

template <template <typename...> class Container, typename... Ts>
auto FirstOrDefault(Container<Ts...>&& results)
    -> decltype(std::move(results.front())) // Use decltype to deduce the return type
{
    //↓ Check if the container is empty and return the appropriate value
    if (results.empty()) {
        return GetDefaultValue<Container<Ts...>>();
    } else {
...
template <typename T, typename = typename std::enable_if<std::is_integral<T>::value>::type> //(template<typename T, typename = std::enable_if_t<std::is_integral_v<T>>>)
