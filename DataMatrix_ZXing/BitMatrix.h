...
#include "DataMatrix_ZXing/Matrix.h"
#include "DataMatrix_ZXing/Point.h"
#include "DataMatrix_ZXing/Range.h"
...
//↓ BitMatrix: A simple, fast 2D array of bits.//
class BitMatrix
{
	int _width = 0;
	int _height = 0;
...
public:
...
#endif
    BitMatrix(int width, int height) : _width(width), _height(height), _bits(width * height, 0) //BitMatrix(int width, int height) : _width(width), _height(height), _bits(width * height, UNSET_V)
	{
    ...
    void flip(int x, int y)     //← Flips the given bit. x : The horizontal component (i.e. which column), y : The vertical component (i.e. which row)
	{
    ...
    //↓ left: The horizontal position to begin at (inclusive), top: The vertical position to begin at (inclusive), width: The width of the region, height: The height of the region
    void setRegion(int left, int top, int width, int height);   //← Sets a square region of the bit matrix to true.
    ...
    //↓ Find the rectangle that contains all non-white pixels (Useful for detection of 'pure' barcodes); return True if this rectangle is at least minWidth x minHeight pixels big
...
//↓ input: matrix to be expanded, width: new width in bits (pixel), height: new height in bits (pixel), quietZone: size of quiet zone to add in modules
BitMatrix Inflate(BitMatrix&& input, int width, int height, int quietZone); //← Inflate : scales a BitMatrix up and adds a quiet Zone plus padding,
                                                                            //return expanded BitMatrix, maybe move(input) if size did not change


//↓ width: new width, height: new height, top: cropping starts at top row, left: cropping starts at left col, subSampling: typically the module size
BitMatrix Deflate(const BitMatrix& matrix, int width, int height, float top, float left, float subSampling);    //← Deflate (crop + subsample) a bit matrix, return deflated input
