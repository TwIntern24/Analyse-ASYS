...
//	return res;
//}
template <typename T, typename RT, int LEN>
constexpr auto BarAndSpaceSum(const T* view) noexcept -> decltype(BarAndSpace<RT>())
{
    BarAndSpace<RT> res;
    for (int i = 0; i < LEN; ++i)
        res[i] += view[i];
...
float IsPattern(const PatternView& view, const FixedPattern<LEN, SUM, false>& pattern, int spaceInPixel = 0, float minQuietZone = 0, float moduleSizeRef = 0)
{
        if(E2E) {//if constexpr (E2E) {
		...
		BarAndSpace<float_t> modSize = {widths[0] / sums[0], widths[1] / sums[1]};
                ...
                decltype(modSize[0]) m, M;
                std::tie(m, M) = std::minmax(modSize[0], modSize[1]);
